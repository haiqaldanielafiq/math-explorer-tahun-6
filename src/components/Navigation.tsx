...
import { BookOpen, PieChart, Award, Lock, Home, Info, Sun, Moon, User, LogOut, ShieldCheck } from 'lucide-react';
...
export function Navbar() {
  ...
  const [user, setUser] = useState<{ name: string; email: string; role: 'ADMIN' | 'STUDENT' } | null>(null);
    ...
    <header className="sticky top-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          {/* Right Controls: Language + Theme + User Profile / Portal Link */}
            ...
            {user ? (
              <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">
                ...
                <div className="hidden sm:block text-left pr-1">
                  <div className="font-bold text-slate-900 dark:text-white text-[11px] truncate max-w-[110px]">
                    {user.name}
                  </div>
                  <div className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1">
                    <span className={`px-1 rounded text-[9px] font-bold ${user.role === 'ADMIN' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/60 dark:text-amber-300' : 'bg-blue-100 text-blue-800 dark:bg-blue-900/60 dark:text-blue-300'}`}>
                ...
                {user.role === 'ADMIN' && (
                  <Link
                    ...
                    className="px-2 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold transition-colors"
                  >
                    CMS
                  </Link>
                )}
