...
export default function ContentEditorPage({
  ...
  return (
    <div className="space-y-8 pb-16 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-900">
            Editor Kandungan: {topic.title} ({topic.code})
          </h1>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-slate-200 hover:bg-slate-300 text-slate-800 text-xs font-bold transition-all"
          >
      ...
      {message && (
        <div className="p-4 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-900 text-sm font-bold">
          {message}
        </div>
      )}
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
        <h2 className="text-lg font-bold text-slate-900">Maklumat Asas Topik DSKP</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Kod DSKP (e.g. 8.1)</label>
            <input
              type="text"
              value={topic.code}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Standard Kandungan</label>
            <input
              type="text"
              value={topic.standardKandungan}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Standard Pembelajaran</label>
            <input
              type="text"
              value={topic.standardPembelajaran}
        ...
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1">Penerangan Ringkas Topik</label>
          <textarea
            rows={2}
      ...
      <div className="space-y-6">
        ...
        {topic.sections?.map((sec, secIdx) => (
          <div key={sec.id} className="bg-white rounded-3xl p-6 border border-slate-200 shadow-md space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <input
                type="text"
                value={sec.title}
                onChange={(e) => handleUpdateSection(sec.id, { title: e.target.value })}
                className="text-lg font-black text-indigo-900 bg-transparent border-b border-transparent hover:border-slate-300 focus:border-indigo-600 focus:outline-none w-full max-w-lg"
              />
              <button
            ...
            {/* Block Items List */}
            <div className="space-y-4 pl-2 sm:pl-4 border-l-2 border-indigo-100">
              {sec.blocks?.map((block) => (
                <div key={block.id} className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-3 relative group">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 bg-slate-200 px-2 py-0.5 rounded">
                      Blok: {block.type}
                    </span>
                    <button
                  ...
                  {block.type === 'callout' && (
                    <div className="space-y-2">
                      <input
                        ...
                        className="w-full p-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-indigo-900"
                      />
                      <textarea
                        rows={2}
                        value={block.body || ''}
                  ...
                  {block.type === 'formula' && (
                    <div className="space-y-2">
                      <input
                        ...
                        value={block.title || ''}
                        onChange={(e) => handleUpdateBlock(sec.id, block.id, { title: e.target.value })}
                        placeholder="Nama Formula (Contoh: Formula Kuantiti Sektor)"
                        className="w-full p-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-amber-900"
                      />
                      <input
                        type="text"
                        value={block.body || ''}
                        ...
                        className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-sm font-mono font-bold"
                      />
                    </div>
                  )}
                  ...
                  {block.type === 'example' && (
                    <div className="space-y-2">
                      <input
                        ...
                        value={block.title || ''}
                        onChange={(e) => handleUpdateBlock(sec.id, block.id, { title: e.target.value })}
                        placeholder="Tajuk Contoh Pengiraan..."
                        className="w-full p-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-emerald-900"
                      />
                      <textarea
                        rows={3}
                        value={block.body || ''}
                        onChange={(e) => handleUpdateBlock(sec.id, block.id, { body: e.target.value })}
                        placeholder="Penerangan langkah demi langkah contoh..."
                        className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-sm"
                      />
                    </div>
                  )}
            ...
            <div className="pt-2 flex flex-wrap items-center gap-2">
              <span className="text-xs font-bold text-slate-500 mr-2">+ Tambah Blok:</span>
              <button
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-700 text-xs font-bold"
              >
                Formula
              </button>
              <button
                onClick={() => handleAddBlock(sec.id, 'example')}
                className="px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-bold"
              >
                Contoh
              </button>
            </div>
