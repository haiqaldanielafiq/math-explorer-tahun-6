...
import { BookOpen, ChevronRight, PieChart } from 'lucide-react';
...
export default async function QuizPage({
  ...
  const quizTitle = getLocalizedText(topic.quiz.title) || `Kuiz ${topicTitle}`;
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
        ...
        <ChevronRight className="w-3.5 h-3.5" />
        <span className="text-amber-600 dark:text-amber-400 font-bold">Kuiz Uji Kefahaman</span>
      </nav>
